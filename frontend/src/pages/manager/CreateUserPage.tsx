...
import type { DoctorRegisterFormValues, RegisterFormValues, StaffRegisterFormValues } from "@/lib/validations/auth";
import CreateUserForm from "@/components/createUserForm/CreateUserForm";

export default function CreateUserPage() {
  const { setTitle, setBreadCrumbs } = useAuthHeader()
  const [userRole, setUserRole] = useState<'ROLE_DOCTOR' | 'ROLE_STAFF'>('ROLE_DOCTOR')
  ...
  const handleStaffSubmit = async (data: RegisterFormValues) => {
    try {
      await createStaff({...data,role:"ROLE_STAFF"});
      ...
      <Card>
        ...
        <CardContent className="pt-6 space-y-4">
          {userRole === 'ROLE_DOCTOR' ? (
            ...
            <CreateUserForm onSubmitCb={handleStaffSubmit} />
          )}
        </CardContent>
      </Card>
    </div>
