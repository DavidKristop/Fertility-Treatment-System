...
export default function PatientScheduleResult() {
  ...
  const [schedule, setSchedule] = useState<
    ScheduleDetailResponse | undefined
  >();
  ...
  useEffect(() => {
    ...
    const fetchList = async () => {
      setLoading(true);
      try {
        const detail = await getStaffScheduleDetail(scheduleId);
  ...
  return (
    <LoadingComponent isLoading={loading}>
      <div className="space-y-6">
        ...
        {/* Contract not signed */}
        {schedule?.treatment &&
          schedule?.treatment.status === "AWAITING_CONTRACT_SIGNED" && (
            <UnSignedContract
              contractUrl={`/patient/contracts/${schedule.treatment.contractId}`}
            />
          )}
        ...
        {schedule?.payment?.some((p) => p.status === "PENDING") && (
          <UnPaidPayment
            payments={schedule.payment.filter((p) => p.status === "PENDING")}
            onClick={(payment) =>
              navigate(`/patient/payments/payment-detail/${payment.id}`)
            }
          />
        )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <User className="h-5 w-5 text-gray-500" />
                    <div>
                      <p className="font-medium">{schedule?.doctor.fullName}</p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <TreatmentStatusBadge
                        status={
                          schedule?.treatment?.status || "Không có thông tin"
                        }
                      />
                      ...
                      {(() => {
                          ...
                          if (
                            ...
                            return (
                              <p className="text-sm text-red-500">
                                ...
                                /{schedule.payment.length} dịch vụ{" "}
                              </p>
                            );
                          }
                        }
                        return (
                          <p className="text-sm text-green-500">
                            Đã thanh toán {schedule?.payment.length}/
                            {schedule?.payment.length} dịch vụ
                          </p>
                        );
                      })()}
